...
%         data(2*counter).type='flight';
%         data(2*counter).support=zeros(2,0);
%         data(2*counter).t=t;
%         data(2*counter).q=[q(:,1) + x0,q(:,2) + y0, q(:,3:4)];
%         data(2*counter).T=zeros(length(q(:,1)),1);
%         [data(2*counter).Fx, data(2*counter).Fy]=forces_flight(q);
%         
...
%         if result_flight==2; performance.terminationmsg=('Body touched ground'); disp('Body touched ground'); result = 0; break; end
    ...
    else; performance.terminationmsg=('Unknown termination cause'); disp('Unknown termination cause');result = 0; break; 
    end
...
% 	end
	...
	function dqdt = EoM_single(~,q)
		...
		dqdt(3,1) = a_spring*q(1)/leg_1 + a_torque*q(2)/leg_1; % ax
		dqdt(4,1) = a_spring*q(2)/leg_1 - a_torque*q(1)/leg_1 - g; % ay
	...
	function dqdt = EoM_double(~,q)
		...
		dqdt(3,1) = a_first_spring*q(1)/leg_1 + a_second_spring*(q(1)-x1)/leg_2 + a_first_torque*q(2)/leg_1 + a_second_torque*(q(2)-y1)/leg_2; % ax
		dqdt(4,1) = a_first_spring*q(2)/leg_1 + a_second_spring*(q(2)-y1)/leg_2 - a_first_torque*q(1)/leg_1 - a_second_torque*(q(1)-x1)/leg_2 - g; % ay
...
% Double stance phase
    ...
    function [Fx, Fy] = forces_single(q)
        ...
        vel_des = repmat(desired_vel, [1,length(q(:,3))] )';
        l_original = repmat(l_0,  [1,length(q(:,3))])';
        gravity = repmat(g, [1,length(q(:,3))])';
        
        T = Kp.*(vel_des - q(:,3)); 

		leg_1 = sqrt(q(:,1).^2+q(:,2).^2); % spring length
		a_spring = K.*(l_original-leg_1)./mass;	% acceleration due to spring
        a_torque = T./(leg_1.*mass); % acceleration due to torque
		Fx = mass*(a_spring.*q(:,1)./leg_1 + a_torque.*q(:,2)./leg_1);
		Fy = mass*(a_spring.*q(:,2)./leg_1 - a_torque.*q(:,1)./leg_1 - gravity);
    ...
    function [Fx, Fy] = forces_double(q)
		...
		leg_1 = sqrt((q(:,1)).^2 + (q(:,2)).^2);				% length of first spring
		leg_2 = sqrt((q(:,1) - x1_vector).^2 + (q(:,2) - y1_vector).^2);	% length of second spring
		a_first_spring = K.*(l_original-leg_1)./mass;					% acceleration due to first spring
		a_second_spring = K.*(l_original-leg_2)./mass;					% acceleration due to second spring
        a_par1 = T./(leg_1.*mass); % acceleration due to torque1
        a_par2 = T./(leg_2.*mass); % acceleration due to torque2
		Fx = mass*(a_first_spring.*q(:,1)./leg_1 + a_second_spring.*(q(:,1)-x1_vector)./leg_2 + a_par1.*q(:,2)./leg_1 + a_par2.*(q(:,2)-y1_vector)./leg_2);
		Fy = mass*(a_first_spring.*q(:,2)./leg_1 + a_second_spring.*(q(:,2)-y1_vector)./leg_2 - a_par1.*q(:,1)./leg_1 - a_par2.*(q(:,1)-x1_vector)./leg_2 - gravity);
